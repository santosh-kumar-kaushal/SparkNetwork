...
class QuestionPagerAdapter(
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        btnNext.setOnClickListener {
            personalityTestData?.let { data -> itemClickListener.invoke(data)
            }
        }
            ...
            questionType.condition?.let {
                it.predicate?.let { it ->
                    if (it.exactEquals.contains(selectedOptions.text.toString())) {
                        val bundle = bundleOf(IF_POSITIVE to questionType.condition?.ifPositive)
                        Navigation.findNavController(view).navigate(R.id.bottomSheet, bundle)
                    }
                }
            }
